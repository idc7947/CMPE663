...
void MFS_print_float(double value) {
	if(value >= -999 && value <= 9999) {
		char ascii[20];
		int len = sprintf(ascii, "%4f", value);	// turn integer into letters
		// set MSB of digit found before a decimal point, to flag setting it in print_str
		for(int ii=1; ii<len; ii++) {
			if(ascii[ii]=='.') {
				ascii[ii-1] |= 0x80;	// flag this digit has a decimal point
				memcpy(&ascii[ii], &ascii[ii+1], len-ii);
			}
		}
		MFS_print_str(ascii);			// turn letters into segments
	}
